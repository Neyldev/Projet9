...
const Page = () => {
  // const {last} = useData() // ancienne ligne
  const { data } = useData() // nouvelle ligne
  const last = data && data.events.sort((a, b) => new Date(b.date) - new Date(a.date))[0] // nouvelle ligne
  return <>
    <header>
      <Menu />
    </header>
